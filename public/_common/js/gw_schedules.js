  ...
  switch (genre) {
    ...
    default:
      if (match_result = genre.match(/^custom_group_(\d+)$/)) {
        ...
        break;
      } else if (match_result = genre.match(/^type_(\d+)$/)) {
        url = '/gw/schedule_props/show_week?s_genre=other&cls=other&type_id='+genre+'&s_date='+s_date;
        break;
      } else if (match_result = genre.match(/^groups_(\d+)$/)) {
        url = '/gw/schedule_props/show_week?s_genre=other&cls=other&type_id='+genre+'&s_date='+s_date;
        break;
      } else {
        alert('実装されていません。')
